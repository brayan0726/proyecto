...
class SelectableVentaLabel(RecycleDataViewBehavior, BoxLayout):
	...
	def refresh_view_attrs(self, rv, index, data):
		...
		self.ids['_total'].text = 'Lps. '+str("{:.2f}".format(data['total']))
...
class InfoVentaPopup(Popup):
	...
	def mostrar(self):
		...
		self.ids.total_dinero.text="Lps. "+str("{:.2f}".format(total_dinero))
...
class VistaVentas(Screen):
	...
	def cargar_venta(self, choice='Default'):
		...
		self.ids.final_sum.text='Lps. '+str("{:.2f}".format(final_sum))
